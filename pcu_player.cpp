...
/*
 * In this file, I decided to break minimax and alpha-beta minimax into two separate functions
 * for ease of readability. In a less pedantic implemenation, I would have consolidated them to 
 * a single function. 
 */

/*
 * Standard minimax. Here instead of a single minimax function, I implemented this using indirect
 * recursion. The function oscillates between maxvalue and minvalue. In the cases that follow, the minimax and 
 * alpha-beta functions first prime the recursion by implicitly doing a max-value step within themselves, then caling minvalue, etc.
 */
void minimax(Node n, vector<int> &move, bool allPlayed)
{
	...
	{
		printStats();
		cout << "Hueristic value for this move: " << (alth ? hueristic2(n) : hueristic(n)) << endl;;
	}
	...
	if(allPlayed)	
	{
		...
		cout << "Choosing move: " << move[0] << " ==> " << move[1] << " value of: " << oldHighest << endl;
	}
	else
	{
		//If there are still pieces to place, then we need to generate children differently. 
...
void alpha_beta(Node n, vector<int> &move, bool allPlayed)
{
	...
	if(trace || fulltrace)
	{
		cout << "Invoking minimax at level: 0 (maximizing), alpha=" << alpha << " beta=" << beta << endl;
		cout << (n.getPlayer() ? "black" : "red") << endl;
		printCompact(n.board);
	}
	...
	if(allPlayed)	
	{
		...
		cout << "Choosing move: " << move[0] << " ==> " << move[1] << " value of: " << alpha << endl;
	}
	else
	{
		generateChildren2(n, children);
...
int maxvalue_ab(Node n, int depth, int alpha, int beta)
{
	...
	if(trace || fulltrace)
	{
		cout << "\nInvoking minimax at level: " << depth << " (maximizing, alpha =" << alpha << " beta =" << beta << ")" << endl;
		cout << (n.getPlayer() ? "black" : "red") << endl;
		printCompact(n.board);
	}
...
int minvalue_ab(Node n, int depth, int alpha, int beta)
{
	...
	if(trace || fulltrace)
	{
		cout << "\nInvoking minimax at level: " << depth << " (minimizing, alpha =" << alpha << " beta =" << beta << ")" << endl;
		cout << (n.getPlayer() ? "black" : "red") << endl;
		printCompact(n.board);
	}
...
/*
 * This function generates children for nodes that have not place all their pegs yet. 
 */
	...
	for(row = 0; row < 5; row++)
	{
		for(col = 0; col < 5; col++)
		{
			...
			if(kid.board[row][col] == '-')
			{
				...
				(n.getPlayer() ? kid.blackpos[piece_count] : kid.redpos[piece_count]) = to;
